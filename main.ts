...
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile`, function (sprite, location) {
    Devil.say("Go into the void", 1000)
})
...
controller.right.onEvent(ControllerButtonEvent.Released, function () {
    ...
    mySprite.setImage(img`
        ...
        `)
    if (Class == 2) {
        mySprite.setImage(assets.image`MageRight`)
    }
})
controller.left.onEvent(ControllerButtonEvent.Released, function () {
    animation.stopAnimation(animation.AnimationTypes.All, mySprite)
    mySprite.setImage(assets.image`BasicLeft`)
    if (Class == 2) {
        mySprite.setImage(assets.image`MageLeft`)
    }
})
scene.onOverlapTile(SpriteKind.Player, sprites.builtin.forestTiles10, function (sprite, location) {
    ...
    scene.setBackgroundImage(img`
        ...
        ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
        `)
    game.splash("It's time for your revenge young warriror")
    game.splash("Defeat the angels", "and complete the levels")
    game.splash("Choose your equipment", "and release mayhem")
    game.splash("Choose a class", "A for Warrior - B for Mage")
...
controller.up.onEvent(ControllerButtonEvent.Released, function () {
    ...
    mySprite.setImage(img`
        ...
        `)
    if (Class == 2) {
        mySprite.setImage(assets.image`MageBehind`)
    }
})
...
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.floorLight5, function (sprite, location) {
    if (Level == 0) {
        Devil.say("Follow me", 500)
        Devil.setPosition(245, 120)
    }
})
let Class = 0
...
mySprite = sprites.create(assets.image`Hero_Melee`, SpriteKind.Player)
...
mySprite.say("What is going on?", 2000)
